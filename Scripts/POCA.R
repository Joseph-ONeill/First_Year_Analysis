...
mds.data <- data.frame(X=mds.values[,1],Y=mds.values[,2], Tenure = Tenure$land_tenure)
mds.data

ggplot(data=mds.data, aes(x=X, y=Y, color = Tenure)) +
  geom_point(size = 5) +
  theme_bw() +
...
Raw_data <- read.csv("Data/All_data_Final.csv")
str(Raw_data)
summary(Raw_data)
nrow(Raw_data)
...
forest.type$colour <- plyr::mapvalues(forest.type$Forest_type, 
                                ...
                                c("red", "blue", "darkslateblue", "green", "brown"))
...
ggplot(biplot.dat, aes(comp1, comp2, color =Forest)) +
  geom_point(size = 5,alpha = 0.5)+ #ggtitle ("Principal Component Analysis (PCoA-PC1 Vs PC2)")+
  xlab("PC1 - Percent Variation Explained")+ 
  ylab("PC2 - Percent Variation Explained")+
  stat_ellipse()+
  theme_bw()
...
Com1 <-data.frame(as.numeric(biplot.dat$comp1))
Com2 <- data.frame(as.numeric(biplot.dat$comp2))


print(hotelling.test(Com1, Com2, shrinkage = FALSE))
...
# ggbiplot(biplot.dat, obs.scale = 1, var.scale = 1, group=Forest,
